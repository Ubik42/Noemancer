...
当前默认 Raster 路径已经启用动态天空、SSR 与 SSGI，并在 RenderLab 取得 D3D12/Vulkan 的固定画面和逐 Pass GPU 时间证据。Native Ray Tracing 已跑通双后端执行闭环，并能由生产 `SceneRenderer` 提交真实场景几何；SDL_GPU 3.4.14 的固定补丁在 Runtime 私有边界提供借用的原生设备/队列与非 cycling 导出纹理。Engine-owned View 与 Shading 合同从项目 Camera、实例、子网格、PBR 标量、方向光和环境光生成稳定 plain-data 计划。D3D12 `/0.3` 复用同一设备，让全帧 RayGen/ClosestHit 消费项目透视相机、几何法线和材质，输出 scene-linear 直接光 + 环境光 + 自发光，再纯 GPU Copy 到 SDL UINT 纹理，由 composite `/0.2` 以 `asfloat`、ACES 和 sRGB 显示。Vulkan RayGen 也已真正消费相机常量，但当前 context 仍只 dispatch/readback 单 texel，尚未接通全分辨率 SDL image/semaphore 呈现。项目可见 RTGI、光追阴影、材质纹理和 VSM 仍在计划中；未进入真实项目管线、跨后端验证和性能证据的能力不会列为已完成。
...
Native Ray Tracing 采用独立 bounded Adapter，不把 D3D12/Vulkan Handle 泄漏进 Scene 或 Agent API。RTX 4080 已在 DXR 1.1 与 Vulkan RT 上真实完成 BLAS/TLAS、Pipeline、SBT、Trace 与 Readback。生产 `SceneRenderer` 通过 Source Adapter、世界空间 Geometry Cache、稳定 instance/primitive identity 和长寿命 Bridge 提交真实场景；`--enable-native-rt-session` 才启用实验路径，默认 Raster 不承担额外成本。最新 640×480 小场景隐藏烟测包含 1,354 个三角形和 23 个材质分组，报告 `cameraShaderConsumed=true`、`shadingResourcesReady=true`、`linearRadianceShaderConsumed=true`、`outputTransferCompleted=true`、`composite.mode=linear-radiance`，同时固定 `claimsRtgi=false`。D3D12 输出的浮点位模式复制进 `R32G32B32A32_UINT` SDL 纹理后，由 composite `/0.2` 正确重解释并独立完成显示映射。Vulkan 已消费相机，但仍是单 texel、无 SDL image/semaphore interop。当前 per-instance/primitive BLAS 映射在 RenderLab 大场景 Debug 首帧超过 90 秒，已作为明确性能回归停止验收；下一步必须改为 source geometry 共享 BLAS、TLAS instance transform 和独立 material index indirection，当前结果不能称为商业性能。
...
商业 Raster 的动态天空、共享 HiZ/History、SSR、SSGI、GPU Scene 遮挡决策和阴影扩展决策均已形成双后端证据。Native D3D12/Vulkan 光追闭环、统一 Receipt、持久 context、生产 SceneRenderer 几何接入、Engine View/Shading plain-data 合同和 D3D12 可见线性直接光已经成立；Vulkan 也已由 RayGen 消费项目相机。当前步骤不是继续堆效果，而是消除 per-instance BLAS 的大场景回归，改为共享 BLAS/TLAS instancing/material indirection，并完成 Vulkan 全帧 storage image 与 SDL synchronization interop。之后才进入 RTGI 独立 History、采样和时空降噪；现阶段 native trace 不能描述成生产 RTGI 画面。
