...
`editor.project-hub-recent-workspaces` 已退出：用户级 Store 使用 `noemancer.recent-workspaces/0.1`、有界列表、严格 UTF-8/JSON、稳定 revision、写前重读和原子替换；损坏状态不阻断 Editor，下一次成功打开可恢复。Runtime 只在 Project Workspace 已成功切换后记录，Editor/Hub 只消费同源列表和不含物理路径的健康投影，Open/New 继续走原 Workspace Authority。跨实例测试与真实 Platformer Headless 打开均证明磁盘状态可重启恢复；最近工程没有进入项目、Package、Scene 或公共 RPC。当前前沿转为 `editor.dpi-localization-visual-matrix`：建立隐藏 DPI/语言/RTL/CJK/缺字矩阵和机器可读布局证据，再以失败样本驱动可再分发 fallback 字体与 cluster-aware 编辑能力。
