...
- 当前切片前两批已经建立通用闭环：`ProjectUiAuthoringSession` 对项目 HUD 源文档执行 expected-revision CAS、dry-run、外部修改冲突检查、原子持久化和可持久化 Undo/Redo；Editor Project UI 面板只发布稳定请求，事务成功后才热应用到 Edit/Play World。RmlUi `button`、Retained UI、Agent `ui.project.action.invoke` 与项目 C# `OnUiAction` 共用规范 binding/action/revision。新增 `components[]`/`componentRef` 直接复用既有 node/presentation 语言，支持声明继承、递归对象合并、数组替换和节点最终覆盖；Runtime 只物化有效节点并保留 `componentChain`，派生字段不能回写源码。Editor 已覆盖 state/presentation/value、Design Token、组件声明及字段级 valid/dirty/disabled/pending/conflict/error。`serve --project PATH` 现在持有持久项目 World/Asset/UI Authority，`ui.project.source.observe/edit` 与 GUI 共用事务实现并在提交后热应用；MCP 可用 `NOEMANCER_PROJECT` 选择该会话。真实 `D:\3D\NoemancerPlatformer` 已完成 source observe 与不改盘 dry-run edit 探针；Debug 全量 CTest 86/86 与 MCP smoke 通过。
- 该切片尚未退出：当前剩余工作是把新增生产字段和组件组合做成清晰的可视交互，并取得必要的隐藏整窗证据；project-attached Agent 已成立，但它明确不是对另一个已打开 Editor 进程的远程控制，也不宣称跨进程共享 Undo journal。视觉收口后进入大型 Sprite/Tilemap 压力；游戏不并入引擎 sample，也不以项目特例抢占通用能力。
