...
当前 `/goal` 目标文本：持续把 `D:\cs\Noemancer`（Noemancer）推进为可独立创建、编辑、调试、打包并发布真实游戏的完整通用引擎，并以 `D:\3D` 下的外部游戏工程作为生产验收客户。每次恢复完整读取并服从仓库规范与四份权威状态，从 `currentFrontier` 首个未阻塞项开始；采用 Sol 主代理集成、最多三个写集互不重叠的 `luna_worker` 并行实际开发，优先代码、测试、Fixture 和证据脚本。子 Agent 只用于能独立形成实质改动的工作，启动后尽快落盘 checkpoint；长时间无产物或协调成本超过实现收益时由主代理中断并接管。每批完成最大连贯子系统改动，再按风险做最小充分验证；公共 Schema、共享 Runtime/World、构建或依赖变化才运行全量测试，Agent ABI 变化才做 MCP 烟测，渲染只使用隐藏 D3D12/Vulkan 证据且不使用 Computer Use。保持 Engine/Runtime Adapter/Editor/Agent Authority 边界，成熟中间件优先，第三方类型不进入持久化或公共协议，游戏规则留在项目 C#。大型 Sprite/Tilemap 生产压力已退出；恢复后从 HD2D 小型外部项目迁移发现真实通用能力缺口，只把可复用能力回写引擎。仅在用户明确暂停、里程碑验收、需要新产品授权/架构冲突或同一外部阻塞连续出现时停止。
...
游戏复刻、兼容引擎和开源游戏迁移是引擎的验收客户，不替代通用引擎主线。当前切片为 `game-migration.hybrid-pixel-hd2d-small-slice`：在 `D:\3D` 新建或复用一个独立的小型 Hybrid Pixel/HD2D 工程，用项目 C#、项目 Scene/Prefab、作者 Sprite/Tilemap、混合 2D/3D 光照、像素 VFX、项目 UI 和 Input 完成一个可玩的短纵切。先由真实制作暴露阻塞，再把稳定、可复用且有独立 Fixture/契约的能力回写引擎；不得把关卡、角色或玩法规则写进引擎 C++，不得把游戏并入引擎 sample。切片出口是 Editor 可打开并手动编辑、源码 Player 与原子 Package Player 行为一致、隐藏 D3D12 画面与结构化运行证据通过、项目无 Native C++、且引擎修改均能脱离该游戏成立。Noemancer Platformer 继续作为基础回归客户；首个 Clean-room 纵切 `D:\3D\StarfallGauntlet` 已按 OpenTyrian 级纵向射击行为范围完成。机器收据及隐藏 D3D12 画面位于 `generated/acceptance/open-2d-migration-current/`。
