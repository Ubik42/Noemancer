...
10. 当前进入 `render.shadow-scalability-vsm-decision`：先扩大当前 CSM/local atlas 的真实压力工作负载，记录 caster/draw/layer/cache invalidation、working set、逐 Pass GPU 时间和画质 failure mode；同时完成 Wicked/Godot 可移植边界与 UE 仅研究边界的 adoption record。只有传统方案在目标 workload 上出现明确分辨率、更新或内存扩展瓶颈，才进入 virtual page 最小原型；不因 VSM 名称流行就复制 UE 实现。
