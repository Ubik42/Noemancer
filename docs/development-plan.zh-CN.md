...
`editor.retained-outliner-and-asset-browser` 已退出：Outliner 与 Asset Browser 都使用 renderer-neutral 有界文档、独立 RmlUi/SDL_GPU Surface、明确输入坐标、稳定选择和真实项目隐藏整窗证据；Asset Browser 还闭合了真实 Thumbnail Artifact 与 >256 资源导航。当前前沿为 `editor.retained-authoring-actions`：把 Outliner 创建/重命名/重设父级/删除，以及 Asset Browser Import/Inspect/Build Preview/Cook 等高频作者操作，逐项接回既有 Scene/Asset Job Authority，并让可见控件、Agent 语义和 receipt 共用动作身份、revision、禁用/忙碌/错误状态。不得复制 Scene、Registry、Selection、Thumbnail Cache、后台 Job 或 Undo；拖放只是一种输入手势，最终仍必须落到同一可测试命令。ImGui 继续承载 Dockspace、Profiler、Render Debugger、长任务和尚未迁移的低层工具。
