...
5. `render.dynamic-sky-atmosphere` 正在推进：engine-owned `noemancer.sky-atmosphere/0.1`、Project `0.2` 可选作者字段、Render Graph v13、transmittance/multi-scattering/sky-view 三张静态 RGBA16F Compute LUT，以及相机 identity 驱动的 32×32×32 RGBA16F camera volume 均已成立。Opaque 后的 Aerial Perspective Pass 通过 depth/world reconstruction 应用 `scene * transmittance + radiance`，天空像素保持不变；Perspective 走真实合成，Orthographic 当前显式 copy-through。Project Settings 的语义作者面板复用同一 plain-data 合同，具备 validation、revision CAS、dry-run、原子持久化与 undo/redo，但尚没有独立的专用 MCP/CLI 命令。D3D12/Vulkan 960×540、60 帧隐藏 GPU timestamp 实测：D3D12 天空/Aerial p95 为 0.0762/0.0491 ms，Vulkan 为 0.1972/0.1009 ms；两后端隐藏画面一致。商业 Raster v1.8 的 Bloom/色彩响应 Fixture 继续使用其版本化受控暗背景，避免天空渐变污染固定 ROI；Render Graph v13 的 1920×1080 Release D3D12 合同位于 `generated/acceptance/commercial-raster-v13-release-d3d12-20260825/`，画质全通过且 CPU Frame p95 为 16.25 ms。两套证据职责不同，均不外推为商业终局。该切片尚未退出：下一步补昼夜时间/天气 Authority、正交相机 Aerial 策略、独立天空固定分辨率 A/B 与低端 analytic 档。
