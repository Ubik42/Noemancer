...
- Asset Browser 的 collection 纵切已迁移到第三个独立 Retained Surface：Model 按稳定 Asset ID 排序，提供 256 项硬上限的查询/游标分页文档，卡片选择 binding 回到现有 Editor Asset Selection；通用 grid 支持 1–12 列、点击、二维方向键、Home/End/Enter，并在真实 Platformer 隐藏 D3D12 整窗中显示 12 个资产和选择高亮。Registry header、搜索/Rescan、Import/Inspect/Preview/Cook、后台 Job、诊断/修复与 Tilemap/AnimationGraph 作者区继续使用原 Authority 和 ImGui 壳，未被复制。下一步补齐 retained `imageSource` 到真实 Thumbnail Artifact 的有界解码/纹理路径，以及 >256 匹配项的可见翻页或测量后虚拟化；完成前不宣称 Asset Browser 全量迁移。
...
当前前沿仍为 `editor.retained-outliner-and-asset-browser`。Outliner 已退出：renderer-neutral 文档、Edit/Play Authority、稳定层级、选择 binding、键盘导航、独立 RmlUi/SDL_GPU Surface、输入路由与真实项目隐藏整窗证据已经贯通，ImGui fallback 只保留尚未迁移的创建/重命名/拖放等作者工具。Asset Browser 的有界分页 Model、通用 grid 交互、独立 Surface、选择回路和混合壳也已成立；当前步骤是把现有 Thumbnail Artifact 安全接到 retained `imageSource`，并为 >256 匹配结果加入可见分页或经压力测量选择虚拟化。不得重复现有 Asset Registry、Selection、Thumbnail Cache、后台 Job 与修复 Authority；ImGui 继续承载 Dockspace、长任务和低层作者工具。
