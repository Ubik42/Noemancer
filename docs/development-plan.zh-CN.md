...
- 当前切片：`hybrid-pixel.large-sprite-tilemap-production-pressure`。以可测生产负载验证长 Sprite 序列、Atlas 碎片/纹理数组决策、Registry→Cook→Package 确定性闭包，以及大型稀疏 Tilemap 的 Chunk 可见集、稳定 GPU Range、碰撞烘焙、内存和增量更新成本；只有证据证明现有 Atlas 路径不足时才引入 texture array。当前决策是先把确定性多页计划接入真实 Atlas 生成、每页内容身份和 Package/Runtime 闭包，再实测局部重建；不因一次线程收益直接更改默认制品策略。完成后进入 `game-migration.hybrid-pixel-hd2d-small-slice`；游戏不并入引擎 sample，也不以项目特例抢占通用能力。
...
- 正式证据 `generated/acceptance/sprite-tilemap-production-pressure-evidence.json` 在真实项目临时副本上通过：真实 2048×2048 Atlas 含 2,048 帧/32 Clips，Registry→Inspect→Cook Apply→Package dry-run 闭合；Tilemap 为 4,096 Chunks、262,144 可寻址/65,536 占用 Cell，裁掉 3,469 Chunks，稳定 range move 为 0，源项目树未改变。KTX2 Adapter 现以完整内容/配方/压缩模式/worker 策略为键提供 256 MiB 有界进程内缓存，并发布分阶段 timing。`generated/acceptance/atlas-cook-performance-current-v3/atlas-cook-performance.json` 的 Release 4096×4096 RGBA8 Basis-LZ 固定负载显示：1 worker 首次 24.698 s，8 worker 自动策略 20.853 s，收益约 15.6%；两者 payload fingerprint 均为 `fnv1a64:80d64e2003c5299b`，同进程重复约 66–99 ms且明确包含输入指纹、计划与 cache lookup，Basis encode 占首次绝大多数时间。该主机的一次等指纹不足以证明跨机器恒等，因此 Registry 生产默认仍为 1 worker，自动并行目前仅存在于 Adapter/证据探针；若以后对项目开放，必须进入明确的 Cook 配方和收据。Sprite 已有不改变 `0.2` 持久格式的确定性多页规划、受影响页面和增量 raw-RGBA 估算；下一批把它接入真实页面生成、每页缓存身份、Package/Runtime 闭包并量化局部重建，再决定拆分 Sprite asset 或 texture array。
