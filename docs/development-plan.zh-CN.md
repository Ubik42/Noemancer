...
9. 当前进入 `render.bindless-gpu-scene-and-occlusion-decision`：用 descriptor churn、CPU submission、visible ratio 与 GPU 时间复审 bindless、GPU Scene 和 HiZ occlusion；不为了热门名称提前引入半套新 Renderer。第一步先量化现有 TextureResourceTable、stable batching、GPU visibility 与共享 HiZ 的真实缺口，再决定 Adopt/Adapt 范围。
