...
5. `render.dynamic-sky-atmosphere` 正在推进：engine-owned `noemancer.sky-atmosphere/0.1`、Project `0.2` 可选作者字段、`render.pass.sky-atmosphere`、transmittance/multi-scattering/sky-view 三张 RGBA16F Compute LUT、参数 identity 驱动的按需重算、direct-raymarch 失败降级，以及 D3D12/Vulkan RenderLab 实拍均已成立。D3D12 固定 60 帧 GPU timestamp 中天空 Pass p50 0.0645 ms、p95 0.2187 ms（当前隐藏性能窗口实际为 575×208，不能外推到 1080p）。该切片尚未退出：下一步补 camera-volume/aerial perspective、昼夜时间/天气 Authority、Editor/Agent 作者入口、固定分辨率 A/B 与低端 analytic 档。
